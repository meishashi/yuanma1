...
// +----------------------------------------------------------------------
// | Author: 3043527191<3043527191@qq.com>
// +----------------------------------------------------------------------
...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'blueseaif'        =>  array('attr'=>'where','level'=>2),
        'elseif'    =>  array('attr'=>'condition','close'=>0),
        'pigelse'    =>  array('attr'=>'where','close'=>0),
		...
		'bluesealist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _blueseaif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'blueseaif');
        $condition   = $this->parseCondition($tag['where']);
        $parseStr  = '<?php if('.$condition.'): ?>'.$content.'<?php endif; ?>';
        return $parseStr;
    }
	...
	public function _pigelse($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'pigelse');
        $condition   = $this->parseCondition($tag['where']);
        $parseStr   = '<?php elseif('.$condition.'): ?>';
        return $parseStr;
    }
	...
	public function _bluesealist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'bluesealist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
