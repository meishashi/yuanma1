...
var G = {
    ...
    domain: {
        w: 'blueseacms.com',
        t: 'http://stc.blueseacms.com',
        k: 'http://www.blueseacms.com/wm-xin-a'
    },
    set: {
        KindEditor_seting: {
            themeType: "simple",
...
G.util.location = {
    ...
    locSelection: function (district) {
        var htm = ['<select name="province" schoolSet><option value="" >请选择</option>'];
        $.each(G.util.area, function (pid, pinfo) {
            htm.push('<option value="' + pid + '">' + pinfo.n + '</option>');
        });
        htm.push('</select> <select name="city" schoolSet><option value="" >请选择</option></select> <select name="district" schoolSet><option value="" >请选择</option></select>');
...
if (!((/blueseacms.com/i.test(window.location)))) G.domain.t = "/static", G.domain.k = "/wm-xin-a";
$(function () {
    G.logic.page.init();
    G.logic.form.init();
})
