var SocketIp = "kf.blueseacms.com";
var TimeInterval;
var getType = 1;
var ajaxAddress = "http://kf.blueseacms.com/";
...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.blueseaid && obj.blueseaid.length > 0) {
                    t.initialize(options, obj.blueseaid, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"blueseaid":"' + obj.blueseaid + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
    ...
    SocketMessage: function (AId, blueseaid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&blueseaid=' + blueseaid + "&AId=" + AId, true);
    },
    StartPolling: function (AId, blueseaid) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, blueseaid);
    },
    GetPolling: function (AId, blueseaid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&blueseaid=' + blueseaid + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.blueseaid) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.blueseaid, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.blueseaid);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, blueseaid, msgcount, socket) {
        var e = this;
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, blueseaid); });
        }
    ...
    socketconnect: function (options, blueseaid) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, blueseaid);
                        }
                    }
                });
            });
