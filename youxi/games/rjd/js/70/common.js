...
function(bt) {
	bt.URL = {
		root: "http://weixin.dingzhishipu.com/youxi",
		getMoreGame: function() {
			bt.dc("more");
			return "http://weixin.dingzhishipu.com/youxi"
		},
		getConcern: function() {
			return "http://weixin.dingzhishipu.com/youxi"
		},
	...
	var loading = function(rate, error) {
		...
		if (loadingDiv) {
			if (error) {
				...
				loadingText.html("a è 1/2  1/2 è?ao|:" + r + "%")
			}
		};
		if (rate >= 1) {
...
function(bt) {
	function ask(cb) {
		if (confirm('a^3ae^3¨"9Gae,,ae"a 3/4 (R)"a?!"i 1/4 a°±a"a>>yenaePèè?"a,aae,,aea|"i 1/4 ')) {
