...
class OrderAction extends userbaseAction {
	...
	public function confirmOrder()//确认收货
	{
	     ...
	     $item=M('item');
	     $item_orders= $item_order->where(array('userId' => $this->visitor->info['id'], 'orderId' => $orderId, 'status' => '3'))->find();
	     if(!is_array($item_orders))
	     {
	     	$this->error('该订单不存在!');
	     }
	     $data['status']=4;//收到货
	     if($item_order->where(array('userId' => $this->visitor->info['id'], 'orderId' => $orderId))->save($data))
	     {
	    	...
	    	foreach ($order_details as $val)
	     	...
	     	$this->redirect('User/index?status='.$status.'&token='.$_GET['token']);
	     }else 
	     {
	     	$this->error('确定收货失败');
	     }
	...
	public function closeOrder()//关闭订单
	{
	 ...
	 $cancel_reason=$_POST['cancel_reason'];
	  ...
	  $item=M('item');
	  $order_detail=M('order_detail');
	  $order=$item_order->where(array('userId' => $this->visitor->info['id'], 'orderId' => $orderId))->find();
	  ...
	  }else 
	  {
	  	...
	  	$data['closemsg']=$cancel_reason;
	   	if($item_order->where(array('orderId' => $orderId))->save($data))//设置为关闭
	   	{
	   		$order_details=$order_detail->where(array('orderId' => $orderId))->select();
	   	...
	   	$item->where('id='.$val['itemId'])->setInc('goods_stock',$val['quantity']);
	   		...
	   		$this->redirect('User/index',array('token'=>$_GET['token']));
	   	}else{
	   		  $this->error('关闭订单失败!');
	   	}
	  }
	...
	public function jiesuan(){//结算
		...
		{
			$this->redirect('Shopcart/index',array('token'=>$_GET['token']));
		}
	}
	...
	public function pay()//出订单
	{
		...
		}else if(isset($_GET['orderId']))
		{
			...
			$orders=$item_order->where(array('userId' => $this->visitor->info['id'], 'orderId' => $orderId))->find();
			if(!is_array($orders))
			$this->_404();
	...
	public  function end()
	{
		if(IS_POST)
		{
			...
			$item_order=M('item_order')->where(array('userId' => $this->visitor->info['id'], 'orderId' => $dingdanhao))->find();
			...
			if($payment_id==2)//货到付款
			{
				...
				if(M('item_order')->where(array('userId' => $this->visitor->info['id'], 'orderId' => $dingdanhao))->data($data)->save())
				{
				$this->redirect('User/index',array('token'=>$_GET['token']));
				}else 
				{
			...
			}elseif ($payment_id==1) //支付宝
			{
				...
				$_SESSION['tmptoken']=$_GET['token'];	
				echo 
					"<script>location.href='wapapli/alipayapi.php?WIDseller_email=".$alipay['alipayname']."&WIDout_trade_no=".$dingdanhao."&WIDsubject=".$dingdanhao."&WIDtotal_fee=".$item_order['order_sumPrice']."'</script>";
