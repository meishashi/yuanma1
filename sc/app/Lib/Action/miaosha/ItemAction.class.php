...
class itemAction extends frontendAction {
    ...
    public function index() {
        ...
        !$id && $this->_404();
        $item_mod = M('item');
        $item = $item_mod->field('id,title,intro,price,info,comments,add_time,goods_stock,buy_num,brand,size,color,startdate,enddate,groupprice')->where(array('id' => $id, 'status' => 1))->find();
        !$item && $this->_404();
