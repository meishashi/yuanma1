...
class bookAction extends frontendAction {
    ...
    public function cate() {
        ...
        if (false === $cate_data = F('cate_data')) {
            $cate_data = D('item_cate')->where('token='.$token)->cate_data_cache();
        }
        ...
        if (false === $cate_list = F('cate_list')) {
            $cate_list = D('item_cate')->where('token='.$token)->cate_cache();
        }
        //分类关系
        if (false === $cate_relate = F('cate_relate')) {
            $cate_relate = D('item_cate')->where('token='.$token)->relate_cache();
        }
        //获取当前分类的顶级分类
        ...
        if ($cate_info['type'] == 0) {
         ...
         // var_dump($cate_relate[$cid]['sids']);exit;
           // $this->waterfall($where, $order);
            ...
            $tag_ids = M('item_cate_tag')->where(array('cate_id' => $cid,'token'=>$token))->getField('tag_id', true);
            if ($tag_ids) {
