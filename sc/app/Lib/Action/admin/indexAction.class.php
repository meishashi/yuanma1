...
class indexAction extends backendAction {
    ...
    public function panel() {
        ...
        $buycount= M('item')->where('status=1 and token="'.$_SESSION['token'].'"')->count();
         $nobuycount= M('item')->where('status=0 and token="'.$_SESSION['token'].'"')->count();
        
        $fukuan= $this->item_order->where('status=1 and token="'.$_SESSION['token'].'"')->count();
        $fahuo= $this->item_order->where('status=2 and token="'.$_SESSION['token'].'"')->count();
        $yfahuo= $this->item_order->where('status=3 and token="'.$_SESSION['token'].'"')->count();
        $this->assign('count',
        array('fukuan'=>$fukuan,
        'fahuo'=>$fahuo,
        'yfahuo'=>$yfahuo,
    ...
    public function left() {
        ...
        if ($menuid) {
            ...
            if ($r = $this->_mod->where(array('often'=>3))->select()) {
              ...
              $left_menu[4] = array('id'=>4,'name'=>'首页广告设置');
