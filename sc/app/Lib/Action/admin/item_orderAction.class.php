    ...
    public function fahuo()
    {
        ...
        if (IS_POST&&$this->_post('orderId','trim')) {
           ...
           /* if( $addid=$mod->add($data) ){
            ...
            if (IS_AJAX) {
            if(count(M('address')->where('status=1 and token="'.$_SESSION['token'].'"')->find())==0)
            	{
             ...
             $deliveryList=	M('delivery')->where('status=1 and token="'.$_SESSION['token'].'"')->order('ordid asc,id asc')->select();//快递方式
              $this->assign('deliveryList',$deliveryList);
              $addressList=M('address')->where('status=1 and token="'.$_SESSION['token'].'"')->order('ordid asc,id asc')->select();//快递方式
