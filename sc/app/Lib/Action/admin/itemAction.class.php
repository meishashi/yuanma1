...
class itemAction extends userAction {
    public function _initialize() {
        ...
        $brandlist= $this->_brand=M('brandlist')->where('status=1 and token="'.$_SESSION['token'].'"')->order('ordid asc,id asc')->select();
        ...
        if (IS_POST) {
           ...
           /*$auid = $this->_post('auid', 'intval');
            ...
            $data['imgs'] = $item_imgs;
			   ...
			   $data['groupno']=$_POST['groupno'];
		   $data['groupprice']=$_POST['groupprice'];
