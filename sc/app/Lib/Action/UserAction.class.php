...
class UserAction extends userbaseAction {
    ...
    public function login() {
        ...
        if (IS_POST) {
            ...
            if (IS_AJAX) {
                ...
                //来路
              //$ret_url = "index.php?g=home&m=Index&a=index&token=mytest";
    ...
    public function logout() {
        ...
        $synlogout = $passport->synlogout();
        //跳转到退出前页面（执行同步操作）
        $this->success(L('logout_successe').$synlogout, U('User/index',array('token'=>$_GET['token'])));
    }
    ...
    */
    public function register() {
        $this->visitor->is_login && $this->redirect('User/index',array('token'=>$_GET['token']));
        if (IS_POST) {
