...
class frontendAction extends baseAction {
    ...
    private  function _index_cate()
    {
            ...
            //推荐到首页的大类
            $index_cate_list = $item_cate_mod->field('id,name,img')->where(array('pid'=>'0' ,'is_index'=>'1', 'status'=>'1','token'=>$_GET['token']))->order('ordid')->select();
			
			
			
            foreach ($index_cate_list as $key=>$val) {
                //推荐到首页的子类
                $where = array('status'=>'1', 'is_index'=>'1','token'=>$_GET['token'], 'spid'=>array('like', $val['id'] . '|%'));
