...
<div class="content">
	<!--zjl start-->
	<?php if($wecha_id != ''): ?><div id="zjl" <?php if($Guajiang['usenums'] != 2): ?>style="display:none"<?php endif; ?> class="boxcontent boxwhite">
		<div class="box">
			<div class="title-red"><span><?php if($Guajiang['usenums'] != 2): ?>恭喜你中奖了<?php else: ?>您已经中过奖了<?php endif; ?></span></div>
			<div class="Detail">
...
$("#save-btn").bind("click",
	function() {
        ...
        if(!telre.test(tel)){
            alert("请输入有效的<?php echo ($Guajiang["renametel"]); ?>");
            return;            
        }
		if(wxname == ''){
			alert("请认真输入微信号");
...
function () {
	...
	$.post('index.php?g=Wap&m=Guajiang&a=exchange', submitData,
	function (data) {
		if (data.success == true) {
			alert(data.msg);
...
window.shareData = {  
            ...
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('Guajiang/index',array('token'=>$token,'id'=>$Guajiang['id']));?>",
