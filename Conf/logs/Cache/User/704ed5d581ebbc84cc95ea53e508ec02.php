...
<li class="<?php if(ACTION_NAME == 'wap_nav'): ?>selectTag<?php endif; ?> " id="tab2"><a href="<?php echo U('Market/wap_nav',array('token'=>$token,'type'=>'market'));?>">商圈首页板块设置</a></li>
  ...
  <div class="cLineB">
    <h4>商家管理</h4>
    <a href="<?php echo U('Market/tenant_set',array('token'=>$token));?>"  class="btn btn-primary btn_submit  J_ajax_submit_btn right" style="margin-top:-30px">
...
<input class="txt lef" type="text" name="like" placeholder="输入查询词">
<input class="btnGrayS" type="submit" value="查询">
</form>
</div>
<div class="clr"></div>
</div>
...
<tr> 
  ...
  <th style=" width:50px;">商家类型</th>
  <th style=" width:50px;">商家区域</th>
  <th style=" width:70px;">入驻时间</th>
  <th style=" width:80px;">操作</th>
</tr>
</thead>
<?php if(is_array($tenant)): $i = 0; $__LIST__ = $tenant;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody>
            <tr>
              ...
              <td class="norightborder">
                <a href="<?php echo U('Market/tenant_set',array('id'=>$vo['id']));?>" >编辑</a> &nbsp;|&nbsp;
                <a href="<?php echo U('Market/tenant_del',array('id'=>$vo['id']));?>" onclick="return(confirm('确定要删除吗？'))">删除</a> 
              </td>
            </tr>
