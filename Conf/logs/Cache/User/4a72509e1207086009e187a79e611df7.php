...
<li class="<?php if(ACTION_NAME == 'wap_nav'): ?>selectTag<?php endif; ?> " id="tab2"><a href="<?php echo U('Market/wap_nav',array('token'=>$token,'type'=>'market'));?>">商圈首页板块设置</a></li>
  ...
  <div class="cLineB">
    <h4>停车区域管理</h4>
    <a href="<?php echo U('Market/park_set',array('token'=>$token));?>"  class="btn btn-primary btn_submit  J_ajax_submit_btn right" style="margin-top:-30px">
...
<thead>
<tr> 
<th style=" width:150px;">停车区域名称</th>
<th style=" width:80px;">车位数量</th>
<th style=" width:150px;">区域介绍</th>
<th style=" width:80px;">设置时间</th>
<th style=" width:50px;">是否使用</th>
<th style=" width:90px;">操作</th>
</tr>
</thead>
<?php if(is_array($park)): $i = 0; $__LIST__ = $park;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tbody><tr>
              <td><?php echo ($item["park_name"]); ?></td>
              <td><?php echo ($item["park_num"]); ?></td>
              <td><?php echo ($item["park_intro"]); ?></td>
              <td><?php echo (date("Y-m-d",$item["add_time"])); ?></td>
              <td>
                <?php if($item["is_use"] == 1): ?>使用中
                <?php else: ?>
                  未启用<?php endif; ?>
                <?php echo ($item["mtitle"]); ?>
              </td>
              <td class="norightborder">
              <a href="<?php echo U('Market/park_set',array('token'=>$item['token'],'park_id'=>$item['park_id']));?>" >编辑</a> &nbsp;|&nbsp;
              <a href="<?php echo U('Market/park_del',array('token'=>$item['token'],'park_id'=>$item['park_id']));?>" onclick="return(confirm('确定要删除吗？'))" >删除</a> </td>
