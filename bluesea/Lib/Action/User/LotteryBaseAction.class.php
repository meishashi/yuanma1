...
class LotteryBaseAction extends UserAction{
    ...
    public function add($type){
        switch ($type){
        case 1: $activeType = 'Lottery';
            break;
        case 2: $activeType = 'Guajiang';
            break;
        case 3: $activeType = 'Coupon';
            break;
        case 4: $activeType = 'LuckyFruit';
            break;
        case 5: $activeType = 'GoldenEgg';
            break;
        case 6: $activeType = 'Research';
            break;
        case 7: $activeType = 'AppleGame';
            break;
        case 8: $activeType = 'Lovers';
            break;
        case 9: $activeType = 'Autumn';
            break;
        }
        if(IS_POST){
        $data = D('lottery');
...
default: $record[$i]['prizeName'] = $r['prize'];
    break;
case 1: $record[$i]['prizeName'] = $data['fist'];
    break;
case 2: $record[$i]['prizeName'] = $data['second'];
    break;
case 3: $record[$i]['prizeName'] = $data['third'];
    break;
case 4: $record[$i]['prizeName'] = $data['four'];
    break;
case 5: $record[$i]['prizeName'] = $data['five'];
    break;
case 6: $record[$i]['prizeName'] = $data['six'];
    break;
case 7: $activeType = 'AppleGame';
    break;
case 8: $activeType = 'Lovers';
...
switch ($rt){
case -1: $this -> error('您的免费活动创建数已经全部使用完,请充值后再使用', U('Home/Index/price'));
break;
case -2: $this -> error('服务器繁忙,请稍候再试');
break;
}
}
}
...
switch ($rt['prize']){
case 1: M('Lottery') -> where(array('id' => $rt['lid'])) -> setDec('fistlucknums');
break;
case 2: M('Lottery') -> where(array('id' => $rt['lid'])) -> setDec('secondlucknums');
break;
case 3: M('Lottery') -> where(array('id' => $rt['lid'])) -> setDec('thirdlucknums');
break;
case 4: M('Lottery') -> where(array('id' => $rt['lid'])) -> setDec('fourlucknums');
break;
case 5: M('Lottery') -> where(array('id' => $rt['lid'])) -> setDec('fivelucknums');
break;
case 6: M('Lottery') -> where(array('id' => $rt['lid'])) -> setDec('sixlucknums');
break;
case 7: $activeType = 'AppleGame';
break;
case 8: $activeType = 'Lovers';
...
foreach ($arr as $field){
    ...
    switch ($field['en']){
    default: break;
    case 'time': case 'sendtime': if ($fieldValue){
            $fieldValue = date('Y-m-d H:i:s', $fieldValue);
        }else{
            $fieldValue = '';
        }
        break;
    case 'wecha_name': case 'prize': $fieldValue = iconv('utf-8', 'gbk', $fieldValue);
        break;
    }
    if ($j < $fieldCount-1){
    echo $fieldValue . "\t";
