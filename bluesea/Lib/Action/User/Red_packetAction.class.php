...
class Red_packetAction extends UserAction
{
    ...
    public function index()
    {
        ...
        foreach ($list as $key => $value) {
            $log                  = M('Red_packet_log')->Distinct(true)->field('wecha_id')->where(array(
                'token' => $this->token,
                'packet_id' => $value['id']
            ))->select();
        ...
        if (IS_POST) {
            if ($prize_db->create()) {
                ...
                if (empty($packet_info)) {
                    ...
                    $this->success('添加成功', U('Red_packet/index', array(
                        'token' => $this->token
                    )));
                } else {
                    $swhere = array(
                    ...
                    if ($offset) {
                        ...
                        $keyword['token']   = $this->token;
                        $keyword['keyword'] = $this->_post('keyword', 'trim');
                        $keyword_db->where(array(
                            'token' => $this->token,
                            'pid' => $this->_post('id', 'intval'),
                            'module' => 'Problem'
                        ))->save($keyword);
                    }
                    $this->success('修改成功', U('Red_packet/index', array(
                        'token' => $this->token
                    )));
                }
            } else {
    ...
    public function del()
    {
        ...
        if (M('Red_packet')->where($where)->delete()) {
            M('Red_packet_prize')->where(array(
                'token' => $this->token,
                'packet_id' => $id
            ))->delete();
            M('Red_packet_log')->where(array(
                'token' => $this->token,
                'packet_id' => $id
            ))->delete();
            M('Red_packet_reward')->where(array(
                'token' => $this->token,
                'packet_id' => $id
            ))->delete();
            ...
            $this->success('删除成功', U('Red_packet/index', array(
                'token' => $this->token
            )));
        }
    }
        ...
        if (IS_POST) {
            if ($prize_db->create()) {
                if (empty($prize_info)) {
                    ...
                    $id                 = $prize_db->add($_POST);
                    $this->success('添加成功', U('Red_packet/prize_list', array(
                        'token' => $this->token,
                        'id' => $packet_id
                    )));
                } else {
                    ...
                    $offset = $prize_db->where($swhere)->save($_POST);
                    $this->success('修改成功', U('Red_packet/prize_list', array(
                        'token' => $this->token,
                        'id' => $packet_id
                    )));
                }
    ...
    public function prize_del()
    {
        ...
        if (M('Red_packet_prize')->where($where)->delete()) {
            $this->success('删除成功', U('Red_packet/prize_list', array(
                'token' => $this->token,
                'id' => $packet_id
            )));
        }
    ...
    public function prize_log()
    {
        ...
        if (!empty($searchkey)) {
            $searchkey         = M('Userinfo')->where(array(
                'truename|wechaname' => $searchkey
            ))->getField('wecha_id');
        ...
        foreach ($list as $key => $value) {
            $list[$key]['wxname'] = M('Userinfo')->where(array(
                'wecha_id' => $value['wecha_id']
            ))->getField('wechaname');
        }
        $this->assign('list', $list);
        $this->assign('packet_id', $packet_id);
    ...
    public function log_del()
    {
        ...
        if (M('Red_packet_log')->where($where)->delete()) {
            $this->success('删除成功', U('Red_packet/prize_log', array(
                'token' => $this->token,
                'id' => $packet_id
            )));
        }
    ...
    public function show_forms()
    {
        ...
        $info['wxname'] = M('Userinfo')->where(array(
    ...
    public function is_ok()
    {
        ...
        $result    = array();
        M('Red_packet_exchange')->where($where)->save(array(
            'status' => '1'
        ));
        $result['err']  = 0;
        $result['info'] = '操作成功！';
    ...
    public function change_del()
    {
        ...
        if (M('red_packet_exchange')->where($where)->delete()) {
            $this->success('删除成功', U('Red_packet/exchange', array(
                'token' => $this->token,
                'id' => $packet_id
            )));
        }
