                ...
                if(!$t_s_setindex->create()){
                   ...
                   $o =  $t_s_setindex->where(array('setid'=>$setid, 'token'=>session('token')))->save($_POST);
                        if($o){
                            ...
                            $this->handleKeyword($setid,'Schoolset',$this->_post('keyword','trim'));
                        ...
                        if($id=$t_s_setindex->data($_POST)->add()){
                            ...
                            $this->handleKeyword($id,'Schoolset',$this->_post('keyword','trim'));

                            $is_cat = M('School_cat')->where(array('token'=>$this->token,'system'=>'1'))->find();
                            if(empty($is_cat)){
                                $this->default_cat();
                            }
