...
class LinkAction extends UserAction{
	...
	public function keywordModules(){
		...
		array('module'=>'Sjm','linkcode'=>'','name'=>'神经猫游戏','sub'=>0,'canselected'=>1,'linkurl'=>'','keyword'=>'神经猫','askeyword'=>1),
	...
	public function modules(){
		...
		array('module'=>'Company','linkcode'=>'http://mo.amap.com/?q='. $company['latitude'].','. $company['longitude'].'&name='. $company['shortname'].'&dev=0','name'=>$this->modules['Company'],'sub'=>1,'canselected'=>1,'linkurl'=>'','keyword'=>'地图','askeyword'=>1),
	...
	public function Company(){
		...
		if ($list){
			foreach ($list as $item){
				...
				array_push($items,array('id'=>$item['id'],'name'=>$item['name'],'linkcode'=>'http://mo.amap.com/?q='. $item['latitude'].','. $item['longitude'].'&name='. $item['name'].'&dev=0','linkurl'=>'','keyword'=>'地图'));
			}
		}
		//
		$this->assign('list',$items);
...
//外链小工具
	...
	public function OutsideLink(){
		//处理小工具数组文件
			$f = include('./WQLCms/Lib/ORG/Func.links.php');
		//取出分类总汇
			$i=0;
