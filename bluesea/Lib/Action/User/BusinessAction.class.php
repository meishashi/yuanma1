...
class BusinessAction extends UserAction
{
    ...
    public function index_add()
    {
        ...
        if (IS_POST) {
            ...
            if (!$check) {
                die($this->error('包含敏感字符,或者是不允许字串!', U('Business/index', array('token' => session('token'), 'type' => $type))));
            } else {
                $_POST['token'] = session('token');
                if (!$t_busines->create()) {
                    die($this->error($t_busines->getError()));
                } else {
                    ...
                    if ('edit' == $status && $bid != '') {
                        ...
                        if ($o) {
                            ...
                            die($this->success('修改成功', U('Business/index', array('token' => session('token'), 'type' => $type))));
                        } else {
                            die($this->error('修改失败', U('Business/index', array('token' => session('token'), 'type' => $type))));
                        }
                    } else {
                        if ($id = $t_busines->data($_POST)->add()) {
                            ...
                            die($this->error('务器繁忙,添加失败,请稍候再试', U('Business/index', array('token' => session('token'), 'type' => $type))));
                        }
                    }
                }
            }
    ...
    public function index_del()
    {
        ...
        if ($result) {
            ...
            die($this->error('非法操作,请稍候再试', U('Business/index', array('token' => session('token'), 'type' => $type))));
        }
    }
    ...
    public function classify_add()
    {
        ...
        if (IS_POST) {
            ...
            $check = filter_var_array($_POST, $filters);
            if (!$check) {
                die($this->error('表单包含敏感字符!'));
            } else {
                $_POST['token'] = session('token');
                if (!$t_busines_main->create()) {
                    die($this->error($t_busines_main->getError()));
                } else {
                    ...
                    if ('edit' == $status && $mid != '') {
                        ...
                        if ($o) {
                            die($this->success('修改成功', U('Business/classify', array('token' => session('token'), 'type' => $type))));
                        } else {
                            die($this->error('修改失败', U('Business/classify', array('token' => session('token'), 'type' => $type))));
                        }
                    } else {
                        if ($id = $t_busines_main->data($_POST)->add()) {
                            ...
                            die($this->error('务器繁忙,添加失败,请稍候再试', U('Business/classify', array('token' => session('token'), 'type' => $type))));
                        }
                    }
                }
            }
    ...
    public function classify_del()
    {
        ...
        if ($result) {
            ...
            die($this->success('删除成功', U('Business/classify', array('token' => session('token'), 'type' => $result['type']))));
        } else {
            die($this->error('非法操作,请稍候再试', U('Business/classify', array('token' => session('token'), 'type' => $type))));
        }
    }
        ...
        if (IS_POST) {
            ...
            if (!$check) {
                ...
                if (!$t_busines_second->create()) {
                    die($this->error($t_busines_second->getError()));
                } else {
                    ...
                    if ('edit' == $status && $sid != '') {
                        ...
                        if ($o) {
                            die($this->success('修改成功', U('Business/project', array('token' => session('token'), 'type' => $type))));
                        } else {
                            die($this->error('修改失败', U('Business/project', array('token' => session('token'), 'type' => $type))));
                        }
                    } else {
                        if ($id = $t_busines_second->data($_POST)->add()) {
                            ...
                            die($this->error('务器繁忙,添加失败,请稍候再试', U('Business/project', array('token' => session('token'), 'type' => $type))));
                        }
                    }
                }
            }
    ...
    public function project_del()
    {
        ...
        if ($result) {
            ...
            die($this->success('删除成功', U('Business/project', array('token' => session('token'), 'type' => $result['type']))));
        } else {
            die($this->error('非法操作,请稍候再试', U('Business/project', array('token' => session('token'), 'type' => $type))));
        }
    }
    ...
    public function poster_add()
    {
        ...
        if (IS_POST) {
            ...
            $check = filter_var_array($_POST, $filters);
            if (!$check) {
                die($this->error('包含特殊字符,请检查后再提交.', U('Business/poster', array('token' => session('token'), 'type' => $type))));
            } else {
                $_POST['token'] = session('token');
                if (!$t_busines_second->create()) {
                    die($this->error($t_busines_second->getError()));
                    ...
                    if ('edit' == $status && $pid != '') {
                        ...
                        if ($o) {
                            die($this->success('修改成功', U('Business/poster', array('token' => session('token'), 'type' => $type))));
                        } else {
                            die($this->error('修改失败', U('Business/poster', array('token' => session('token'), 'type' => $type))));
                        }
                    } else {
                        if ($id = $t_busines_second->data($_POST)->add()) {
                            ...
                            die($this->error('务器繁忙,添加失败,请稍候再试', U('Business/poster', array('token' => session('token'), 'type' => $type))));
                        }
                    }
                }
            }
    ...
    public function poster_del()
    {
        ...
        if ($result) {
            ...
            die($this->success('删除成功', U('Business/poster', array('token' => session('token'), 'type' => $result['type']))));
        } else {
            die($this->error('非法操作！请稍候再试', U('Business/poster', array('token' => session('token'), 'type' => $type))));
        }
    }
    ...
    public function comments_add()
    {
        ...
        if (IS_POST) {
            ...
            $check = filter_var_array($_POST, $filters);
            if (!$check) {
                die($this->error('表单包含不允许字符.', U('Business/comments', array('token' => session('token'), 'type' => $type))));
            } else {
                $_POST['token'] = session('token');
                if (!$t_busines_comment->create()) {
                    die($this->error($t_busines_comment->getError()));
                } else {
                    ...
                    if ('edit' == $status && $cid != '') {
                        ...
                        if ($o) {
                            die($this->success('修改成功', U('Business/comments', array('token' => session('token'), 'type' => $type))));
                        } else {
                            die($this->error('修改失败', U('Business/comments', array('token' => session('token'), 'type' => $type))));
                        }
                    } else {
                        if ($id = $t_busines_comment->data($_POST)->add()) {
                            ...
                            die($this->error('服务器繁忙,添加失败,请稍候再试', U('Business/comments', array('token' => session('token'), 'type' => $type))));
                        }
                    }
                }
            }
    ...
    public function comments_del()
    {
        ...
        if ($result) {
            ...
            die($this->success('删除成功', U('Business/comments', array('token' => session('token'), 'type' => $result['type']))));
        } else {
            die($this->error('非法操作！请稍候再试', U('Business/comments', array('token' => session('token'), 'type' => $type))));
        }
    }
