	...
	public function pay(){
		...
		if ($alipayConfig['paytype']=='Weixin'){
			header('Location:/wxpay/index.php?g=Wap&m='.$alipayConfig['paytype'].'&a=pay&price='.$price.'&orderName='.$orderName.'&single_orderid='.$orderid.'&showwxpaytitle=1&from='.$from.'&token='.$this->token.'&wecha_id='.$this->wecha_id);
                    ...
                    $pay_action = 'pay';
                    if ($alipayConfig['paytype'] == 'kuaiqian') {
                        $alipayConfig['paytype'] = 'Alipaytype';
                        $pay_action = 'pay_kq';
                    }//echo $pay_action;exit;
			header('Location:/index.php?g=Wap&m='.$alipayConfig['paytype'].'&a=pay&type_kq='.$pay_action.'&price='.$price.'&orderName='.$orderName.'&single_orderid='.$orderid.'&from='.$from.'&token='.$this->token.'&wecha_id='.$this->wecha_id);
		}
	}
}
?>