...
class WeixinAction extends BaseAction{
	...
	public function __construct(){
		//蓝海之蓝bluesea更新微支付功能,QQ:3043527191
		parent::_initialize();
		...
		if(!empty($payConfig['info'])){
			$payConfigInfo = unserialize($payConfig['info']);
			$this->payConfig= $payConfigInfo['weixin'];
			if(ACTION_NAME == 'pay' && empty($payConfig['paysignkey'])){
				$this->new_pay();
				exit;
			}
		}else if(!empty($payConfig['mchid'])){
			$this->payConfig['appid'] = $payConfig['appid'];
			$this->payConfig['mchid'] = $payConfig['mchid'];
			$this->payConfig['key'] = $payConfig['paysignkey'];
			$this->payConfig['appsecret'] = $payConfig['appsecret'];
			if(ACTION_NAME == 'pay'){
				$this->new_pay();
				exit;
			}
	...
	public function new_pay(){
		...
		//使用jsapi接口
		$jsApi = new JsApi_pub($this->payConfig['appid'],$this->payConfig['mchid'],$this->payConfig['key'],$this->payConfig['appsecret']);
		...
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$url = $jsApi->createOauthUrlForCode(urlencode(C('site_url').'/wxpay/index.php?g=Wap&m=Weixin&a=new_pay&price='.$_GET['price'].'&orderName='.$_GET['orderName'].'&single_orderid='.$_GET['single_orderid'].'&showwxpaytitle=1&from='.$_GET['from'].'&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id']));
...
//蓝海之蓝bluesea更新微支付功能,QQ:3043527191
		...
		//使用统一支付接口
		$unifiedOrder = new UnifiedOrder_pub($this->payConfig['appid'],$this->payConfig['mchid'],$this->payConfig['key'],$this->payConfig['appsecret']);	
		...
		$returnUrl = C('site_url').'/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid;
